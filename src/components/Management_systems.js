import React from 'react';

export default function Management_systems() {
  return (
    <div>
      <div className="section about-section section-padding">
        <div className="container">
          {/* About Wrap Start */}
          <div className="about-wrap">
            <div className="row">
              <div className="col-lg-6">
                {/* About Thumb Wrap Start */}
                <div className="about-thumb-wrap">
                  <div className="about-thumb-small">
                    <img src="assets/images/service/pixlr-bg-result234.png" alt="" height={250} />
                  </div>
                  <div className="about-thumb-big">
                    <img src="assets/images/service/pixlr-bg-result11231.png" alt="" height={300} width={300} />
                  </div>
                  <div className="about-thumb-shape">
                    <img src="assets/images/ab-shape.png" alt="" />
                  </div>
                  {/* <div class="play-btn">
                              </div> */}
                </div>
              </div>
              <div className="col-lg-6">
                {/* About Content Start */}
                <div className="about-content">
                  <div className="section-title">
                    <h3 className="sub-title">management systems</h3>
                    <h2 className="title">Our team has extensive experience in building custom Management systems that ranges from retail, healthcare to ERP systems.</h2>
                  </div>
                  <p>LancolaTech active team ca dive into our business, explore your needs, understand capabilities and requirements and build a customized interacive system to unlock mobility and full potential in your company.</p>
                </div>
                {/* About Content End */}
              </div>
            </div>
          </div>
          {/* About Wrap End */}
        </div>
      </div>
      {/*  Skill End */}
      {/* Cta Start */}
      <div className="section cta-section-3 section-padding-03" style={{backgroundImage: 'url(assets/images/bg/counter-bg.jpg)'}}>
        <div className="container">
          {/* Cta Wrap Start */}
          <div className="cta-wrap-3">
            <div className="row justify-content-center">
              <div className="col-lg-8">
                <div className="cta-content text-center">
                  <h2 className="title">Make a great project with us</h2>
                  <p>To make requests for further information, <a href="contact.html">Contact Us</a></p>
                </div>
              </div>
            </div>
          </div>
          {/* Cta Wrap End */}
        </div>
      </div>
    </div>
  );
}